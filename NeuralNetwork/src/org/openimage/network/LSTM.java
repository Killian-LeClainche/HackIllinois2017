...
/**
 * LSTM stands for Long-Short Term Memory. An LSTM node is centered around a
 * recurrent neuron (points to itself). It receives an input which is gated by
 * an input neuron, and outputs a signal which is gated by an output neuron.
 * Additionally, the value of the memory neuron is gated by a keep neuron. Each
 * of the gate neuron uses a step function for squashing instead of a sigmoid
 * function. Thus, the I/O of the memory node is opened or closed by multiplying
...
public class LSTM implements Node
{
	...
	/*
	 * public Node keep; public Node write; public Node read; public Node
	 * memory; public Node input; public Node output;
	 */
	...
	/**
	 * @return All external weights corresponding to all external inputs of the
	 *         LSTM.
	 */
	 ...
	 * @param weight
	 *            The weight to be multiplied by the node input.
	 */
	...
	/**
	 * Add node input to a gate with a missing input, otherwise a random gate. A
	 * normalized weight is generated and set.
	 * 
	 * @param node
	 *            The node to be connected.
	 */
	public void addIncomingNode(Node node)
	{
		...
		for (int i = 0; i < INPUT + 1; i++)
		{
			...
			{
				gate.getIncomingNodes().add(node);
				gate.getIncomingWeights().add((1.0 / Math.sqrt(gate.getIncomingNodes().size())) * Math.random());
				return;
			}
		}

		gates[minIndex].getIncomingNodes().add(node);
		gates[minIndex].getIncomingWeights()
				.add((1.0 / Math.sqrt(gates[minIndex].getIncomingNodes().size())) * Math.random());

		/*
		 * int seed = org.openimage.Param.rng.nextInt(3 + 1);
		 * gates[seed].getIncomingNodes().add(node);
		 * gates[seed].getIncomingWeights().add((1 /
		 * Math.sqrt(gates[seed].getIncomingNodes().size())) * Math.random());
		 */
	}
	...
	/**
	 * Computes the current output value of the LSTM using the write gate and
	 * memory node. Then, the memory is squashed, and the rest of the gets are
	 * activated to prepare for the next cycle.
	 * 
	...
	public double activate()
	{
		...
		assert (getKeep().getIncomingNodes().size() > 0 && getRead().getIncomingNodes().size() > 0
				&& getWrite().getIncomingNodes().size() > 0 && getInput().getIncomingNodes().size() > 0
				&& getMemory().getIncomingNodes().size() > 0 && getOutput().getIncomingNodes().size() > 0);
		...
		getMemory().activate(
				getInput().getValue() * getRead().getPrevious() + getMemory().getValue() * getKeep().getValue());
	 ...
	 * 
	 * @param input
	 *            The input to be gated and squashed.
	...
	{
		return "LSTM { gates: " + Arrays.toString(this.gates) + ", previous: " + this.previous + ", value: "
				+ this.value + " }";
	}
}