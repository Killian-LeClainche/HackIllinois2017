...
public class Layer
{
	 ...
	 * @param lstmCount
	 *            The number of those nodes which should be LSTM's.
	 */
	 ...
	 * 
	 * @param weights
	 *            The weights of the synapses corresponding from top to bottom.
	 */
	 ...
	 * 
	 * @param before
	 *            The layer that preceeds the current one.
	 */
	...
	/**
	 * Connect synapses from each Node in Before pointing to each Node in After.
	 * Also connect their respective weights.
	 * 
	 * @param before
	 *            The layer the preceeds this one.
	 * @param weights
	 *            The weights that correspond to the Nodes in this layer from
	 *            top to bottom.
	 */
	public void connectAfter(Layer before, List<Double> weights)
	{
		...
		/*
		 * for(int i = 0; i < before.nodes.size(); i++) { for(int j = 0; j <
		 * nodes.size(); j++) {
		 * nodes.get(j).addIncomingNode(before.nodes.get(i), weights.get(i + j *
		 * before.nodes.size())); } }
		 */
	}
